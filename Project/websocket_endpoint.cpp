...
namespace kagula
{
    ...
    public:
        ...
        connection_metadata(websocketpp::connection_hdl hdl, std::string uri, CWnd* pWnd)
            : m_hdl(hdl)
            , m_status("Connecting")
            , m_uri(uri)
            , m_server("N/A")
        ...
        void on_open(ws_client *client, websocketpp::connection_hdl hdl) {
            ...
            CLed_toolDlg* pDlg = (CLed_toolDlg*)(m_pWnd);
            pDlg->onWebOpen();
        }
        ...
        void on_close(ws_client *client, websocketpp::connection_hdl hdl) {
			...
			CLed_toolDlg* pDlg = (CLed_toolDlg*)(m_pWnd);
            pDlg->onWebClose(con->get_remote_close_code(), websocketpp::close::status::get_string(con->get_remote_close_code()),
...
//             }
            ...
            if (msg->get_opcode() == websocketpp::frame::opcode::text) 
            {
                pDlg->onWebMessage(msg->get_payload());
            }
            else 
            {
                pDlg->onWebMessage(websocketpp::utility::to_hex(msg->get_payload()));
            }
        }
    ...
    int websocket_endpoint::connect(std::string const & uri, CWnd* pWnd) {
        websocketpp::lib::error_code ec;
        ...
        g_wsClientConnection = websocketpp::lib::make_shared<connection_metadata>(pConnection->get_handle(), uri, pWnd);
